...
export let USE_MOCK =
  String(import.meta.env.VITE_USE_MOCK_API ?? (BASE ? "false" : "true"))
    .toLowerCase() === "true";


export const ApiMode = { isMock: () => USE_MOCK, base: () => BASE };
...
const Real = {
  ...
  createVideowall: (payload) =>
    req("/videowalls", { method: "POST", body: payload }),
  updateVideowall: (id, payload) =>
    req(`/videowalls/${encodeURIComponent(id)}`, {
      method: "PUT",
      body: payload,
    }),
  deleteVideowall: (id) =>
    req(`/videowalls/${encodeURIComponent(id)}`, { method: "DELETE" }),

  // Scenes
  
  listScenes: (vwId, params = {}) =>
    req(`/videowalls/${encodeURIComponent(vwId)}/scenes${q(params)}`),
  listScenesFull: (vwId, params = {}) =>
    req(`/videowalls/${encodeURIComponent(vwId)}/playlist${q(params)}`),
  ...
  updateScene: (id, payload) =>
    req(`/scenes/${encodeURIComponent(id)}`, { method: "PUT", body: payload }),
  deleteScene: (id) =>
    req(`/scenes/${encodeURIComponent(id)}`, { method: "DELETE" }),

  // Alerts + presence

  alertSummary: () => req(`/alerts/summary`),
  criticalState: () => req(`/alerts/critical`),
  visitorPresence: (videowall) =>
    req(
      `/visitor/presence?videowall=${encodeURIComponent(videowall)}`
    ),
  ...
  freeze: (vwId, sceneId) =>
    req(`/player/${encodeURIComponent(vwId)}/freeze`, {
      method: "POST",
      body: { sceneId },
    }),
  unfreeze: (vwId) =>
    req(`/player/${encodeURIComponent(vwId)}/freeze`, { method: "DELETE" }),
};
...
const Mock = {
  ...
  updateVideowall: async (id, { name }) => {
    ...
    LS.write(
      LS.vwListKey,
      list.map((v) => (v.id === id ? { ...v, name: name ?? v.name } : v))
    );
    return { ok: true };
  },
  ...
  updateScene: async (id, payload) => {
    ...
    LS.write(
      LS.idxKey(vw),
      idx.map((s) => (s.id === id ? updatedSummary : s))
    );
    const full = {
      id,
      videowall_id: vw,
  ...
  alertSummary: async () => {
    const t = (Date.now() / 10000) | 0;
    const mod = (n) => t % n;
    return {
      open: 8 + mod(4),
      closed_without_reason: mod(3),
      closed_with_reason: 5 + mod(3),
      critical: 0,
    };
  },
  ...
  freeze: async (vwId, sceneId) => {
    LS.write(LS.freezeKey(vwId), { frozen: true, sceneId });
    return { ok: true };
  },
  unfreeze: async (vwId) => {
    LS.write(LS.freezeKey(vwId), { frozen: false, sceneId: null });
    return { ok: true };
  },
};
...
// ---- Unified API (real with auto-fallback to mock) ---- //

async function tryRealThenMock(method, ...args) {
  if (!USE_MOCK && BASE) {
    try {
      return await Real[method](...args);
    } catch (e) {
      console.warn("API offline, falling back to MOCK:", e?.message || e);
      USE_MOCK = true;
...
export const Api = {
  ...
  me: (...a) => tryRealThenMock("me", ...a),
  login: (...a) => tryRealThenMock("login", ...a),
  logout: (...a) => tryRealThenMock("logout", ...a),

  // Videowalls
  
  listVideowalls: (...a) => tryRealThenMock("listVideowalls", ...a),
  createVideowall: (...a) => tryRealThenMock("createVideowall", ...a),
  updateVideowall: (...a) => tryRealThenMock("updateVideowall", ...a),
  deleteVideowall: (...a) => tryRealThenMock("deleteVideowall", ...a),

  // Scenes
  
  listScenes: (...a) => tryRealThenMock("listScenes", ...a),
  listScenesFull: (...a) => tryRealThenMock("listScenesFull", ...a),
  getScene: (...a) => tryRealThenMock("getScene", ...a),
  createScene: (...a) => tryRealThenMock("createScene", ...a),
  updateScene: (...a) => tryRealThenMock("updateScene", ...a),
  deleteScene: (...a) => tryRealThenMock("deleteScene", ...a),

  // Alerts + presence
  
  alertSummary: (...a) => tryRealThenMock("alertSummary", ...a),
  criticalState: (...a) => tryRealThenMock("criticalState", ...a),
  visitorPresence: (...a) => tryRealThenMock("visitorPresence", ...a),

  // Player freeze
  
  getFreeze: (...a) => tryRealThenMock("getFreeze", ...a),
  freeze: (...a) => tryRealThenMock("freeze", ...a),
  unfreeze: (...a) => tryRealThenMock("unfreeze", ...a),
};
